...
import { deptQuery, spersonnelQuery, searchPersonnel } from '../../../service/systemApi';
import { generatorTableKey, hideLoading, isEnglish, showLoading, systemErgodicRoot } from '../../../utils/utils';
...
var fields = language.userManage.staffManage.fields;
...
var UserSelectorForm = function UserSelectorForm(_ref, ref) {
  ...
  var _useState = useState([]),
      ...
      selectedNodeKey = _useState2[0],
      setSelectedNodeKey = _useState2[1];

  var _useState3 = useState([]),
      _useState4 = _slicedToArray(_useState3, 2),
      selectedNodeRows = _useState4[0],
      setSelectedNodeRows = _useState4[1];

  var _useState5 = useState([]),
      _useState6 = _slicedToArray(_useState5, 2),
      rowKeys = _useState6[0],
      setRowKeys = _useState6[1];
  ...
  var _useState13 = useState([]),
      ...
      expandKeys = _useState14[0],
      setExpandKeys = _useState14[1];
      ...
      Ok: function Ok() {
        ...
        if (rows.length > 0) {
          if (editor === 'UserSelector') {
            ...
            if (selectMap === 1) {
              var mapList = selectedNodeRows[0];
              result[tfield] = isEnglish ? mapList.name : mapList.ename;
            }

            result[fieldName] = rows[0].suName;
            result['userFieldValue'] = rows[0].name + ':' + rows[0].suName;
            result['userFieldName'] = fieldName;
          } else {
            var arr = rows.map(function (item) {
              return item.suName;
            });
            result[fieldName] = arr.join(',');
            result['userFieldValue'] = rows.map(function (item) {
              return item.name + ':' + item.suName;
            }).join(',');
            result['userFieldName'] = fieldName;
          }
        }
  ...
  var loop = function loop(data) {
    ...
    return data.map(function (item) {
      var name = item.name,
          id = item.id,
          ename = item.ename;
      var NAMEPRO = isEnglish ? ename || name : name || ename;
      var title = /*#__PURE__*/React.createElement("span", null, NAMEPRO);
      var props = {
        key: id,
        title: title,
        dataRef: item
  ...
  var onSelect = function onSelect(selectedKeys, e) {
    ...
    setSelectedNodeKey([selectedNode.id]);
    setSelectedNodeRows([selectedNode]);
    setRowKeys([]);
    setRows([]);
    showLoading();
    spersonnelQuery({
  ...
  var changePage = /*#__PURE__*/function () {
    var _ref2 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(page, pageSize, keyWord) {
      var state;
          ...
          switch (_context.prev = _context.next) {
            case 0:
              ...
              return searchPersonnel({
                deptId: !selectedNodeRows[0] || selectedNodeRows[0].pid === '0' ? 0 : selectedNodeRows[0].id,
                page: page,
                pageSize: pageSize,
                keyWord: keyWord
              }).then(function (data) {
    ...
    return function changePage(_x, _x2, _x3) {
      return _ref2.apply(this, arguments);
    };
  }();

  var rowSelection = {
    type: editor === 'UserSelector' ? 'radio' : 'checkbox',
  ...
  var userProp = {
    ...
    onSearch: selectedNodeKey.length ? function (keyWord) {
      return changePage(page, pageNumber, keyWord);
    } : null,
    paging: paging
  };
  ...
  }, /*#__PURE__*/React.createElement(TableWithBtn, userProp))));
};
