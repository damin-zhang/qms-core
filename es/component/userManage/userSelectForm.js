...
var UserSelectorForm = function UserSelectorForm(_ref, ref) {
  ...
  var _useState3 = useState([]),
      ...
      rowKeys = _useState4[0],
      setRowKeys = _useState4[1];
  ...
  useEffect(function () {
    if (editor) {
      deptGetUserEditor().then(function (res) {
        if (res) {
          setLoading(true);
      ...
      Ok: function Ok() {
        ...
        if (rows.length > 0) {
          if (editor === 'UserSelector') {
            ...
            result['NAME'] = arr;
            result['NAMEr'] = rows.map(function (item, index) {
              return item.NAME;
            });
          }
        }
  ...
  var onSearch = function onSearch(value) {
    ...
    setExpandKeys([pid[0]].concat(_toConsumableArray(expandedKeys))); // 滚动第一个搜索项到视野中
