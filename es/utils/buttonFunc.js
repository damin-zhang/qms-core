...
import { judgeModel, hideLoading, isEnglish } from './utils';
export var clickBtn = function clickBtn(object, engine, model, callback) {
  ...
  var objectNo = object.objectNo,
      ...
      name = object.name;
  caption = caption ? caption : isEnglish ? enName || name : name || enName;
  ...
  if (implclass === 'demo.zc.ActStartProcess') {
    ...
    if (wType === 'browseFiles') {
      if (!selectedRows[0].FLOCATION) {
        var msg = isEnglish ? "There are currently no files to preview" : "\u5F53\u524D\u6CA1\u6709\u53EF\u9884\u89C8\u7684\u6587\u4EF6";
        message.warn(msg);
        return;
      ...
      if (!selectedRows[0].FLOCATION) {
        var _msg = isEnglish ? "There are currently no files available for download" : "\u5F53\u524D\u6CA1\u6709\u53EF\u4E0B\u8F7D\u7684\u6587\u4EF6";
...
export var confirmBtn = function confirmBtn(_ref, clickedMenu, namespace, pageId, tabCountData, callback, dataTotal) {
  ...
  var dva = getDvaApp(); // wType:browseFiles 文件预览
